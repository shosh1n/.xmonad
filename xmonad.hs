...
myAditionalKeys =
  ...
  , ("M-S-d", spawn "killall xmobar; xmonad --recompile; xmonad --restart")
...
myStartupHook :: X ()
myStartupHook = do
    spawnOnce "feh --bg-fill ~/Pictures/Metroid.png &"
    spawnOnce "picom &"
...
myXmobarPP s  = filterOutWsPP [scratchpadWorkspaceTag] . marshallPP s $ def
  ...
  where
    ...
    layoutColorIsActive n l = do
      c <- withWindowSet $ return . W.screen . W.current
      if n == c then wrapL "<icon=/home/shoshin/.config/xmonad/xmobar/icons/" "_selected.xpm/>" l else wrapL "<icon=/home/shoshin/.config/xmonad/xmobar/icons/" ".xpm/>" l
